...
<div style='background:#0e1520;border:1px solid #1e2d42;border-radius:8px;
  ...
  <div style='flex:1;min-width:200px'>
    ...
    <div style='font-size:0.9rem;color:#ffd234;font-weight:600'>Expert Knowledge Base + DAG Propagation</div>
    <div style='font-size:0.75rem;color:#5a7494;margin-top:2px'>NetworkX weighted DAG — shock propagates via topological sort</div>
  </div>
...
Accuracy is measured using <b style='color:#e8eef5'>Mean Absolute Percentage Error (MAPE)</b> —
a standard metric in forecasting literature. A lower MAPE confirms the system's reliability as a Decision Support tool.
...
def render_validation_tab(event_key, tab):
    ...
    with tab:
        st.markdown(f"<p style='color:#5a7494;font-family:monospace;font-size:0.78rem;margin-bottom:16px'>Period: {period} &nbsp;|&nbsp; Metric: MAPE (Mean Absolute Percentage Error)</p>", unsafe_allow_html=True)
        ...
        for sector, pred, real_val, source in rows:
            abs_err = abs(pred - real_val)
            mape = (abs_err / abs(real_val)) * 100 if real_val != 0 else 0
            total_mape += mape
            c, lbl = accuracy_color(mape)
            ...
            <tr style='border-bottom:1px solid #131d2a'>
              ...
              <td style='padding:11px 14px;text-align:center'>
                <span style='font-family:monospace;font-size:0.82rem;font-weight:700;
                  color:{c};background:{"rgba(124,255,107,0.08)" if mape<=6 else "rgba(255,59,92,0.08)"};
                  padding:3px 8px;border-radius:4px'>MAPE {mape:.1f}% — {lbl}</span>
              </td>
              <td style='padding:11px 14px;color:#3a5070;font-size:0.75rem;line-height:1.4'>{source}</td>
            </tr>"""

        avg_err = total_mape / len(rows)
        avg_c, avg_lbl = accuracy_color(avg_err)

        # Summary bar
        accuracy_pct = max(0, 100 - avg_err)
        st.markdown(f"""
        <div style='background:#0e1520;border:1px solid #1e2d42;border-radius:8px;
        ...
        <div style='margin-top:12px;padding:10px 16px;background:#090e18;border-radius:6px;
          ...
          Ministry of Petroleum India. Using Mean Absolute Percentage Error (MAPE) as the
          evaluation metric, the system achieves an average MAPE of
...
with val_tabs[3]:
    ...
    for ev_key, ev_data in VALIDATION_DATA.items():
        rows = ev_data["rows"]
        mape = sum((abs(r[1]-r[2])/abs(r[2]))*100 for r in rows if r[2]!=0) / len(rows)
        event_names.append(ev_key[:25])
        event_errors.append(round(mape, 2))
        event_accuracies.append(round(max(0, 100 - mape), 1))
    ...
    m3.metric("Overall MAPE", f"{overall_avg_err:.2f}%", ol)
    ...
    <div style='background:#0e1520;border:1px solid #1e2d42;border-left:4px solid #7cff6b;
      ...
      <div style='font-size:0.88rem;color:#8aaccc;line-height:1.8'>
        ...
        pairs, EcoShock achieved an overall Mean Absolute Percentage Error (MAPE) of <b style='color:#7cff6b'>{overall_avg_err:.2f}%</b>
        ...
        Ministry of Tourism India. This validation confirms that a rule-based expert-knowledge DSS
...
if any([oil_shock, food_shock, unemp_shock, currency_drop, interest_rise]):
    ...
    <div style='background:#0e1520;border:1px solid #1e2d42;border-left:4px solid #ffd234;
      ...
      interest rate hike {interest_rise} bps — confirming cascading effects in the
      rule-based sector impact model."
      </span>
    </div>""", unsafe_allow_html=True)
...
arch_rows = [
    ...
    ("3", "CORE ENGINE",      "#a855f7",
     "Graph Propagation Engine builds a 12-node Directed Acyclic Graph (DAG). A shock severity value (1–30 scale) is assigned to the root node (Global Event) and propagates through weighted directed edges using NetworkX topological sort — each downstream node accumulates impact proportional to its edge weight. This produces an impact score for each sector and social class. Separately, a Sensitivity Analyser applies 5-parameter (oil, food, unemployment, currency, interest) stress testing with sector-specific multipliers derived from economic transmission theory."),
...
<div style='margin-top:10px;padding:12px 16px;background:#090e18;border-radius:6px;
  ...
  Layer 1 accepts preset or custom scenario text. Layer 2 applies a keyword-weighted scoring
  engine to assign a shock severity index. Layer 3 (Core Engine) implements a 12-node weighted
  Directed Acyclic Graph (DAG) using NetworkX topological sort to propagate shock values
  through interconnected economic sectors and social classes. Layer 4 renders nine output
  components including sector impact cards, class comparison tables, sensitivity charts,
...
<br><br>
<b style='color:#e8eef5'>Core Algorithm — Two-Layer Architecture:</b><br>
EcoShock uses a two-layer computation model. <b style='color:#00e5ff'>Layer 1 (Knowledge Base):</b>
A curated expert knowledge base encodes sector impact magnitudes for each scenario,
derived from RBI Annual Reports, World Bank Commodity Outlooks, MOSPI CPI data, and
IMF World Economic Outlook publications. Each sector's impact value is sourced and
validated against real-world reported figures.
<b style='color:#00e5ff'>Layer 2 (Graph Propagation):</b>
The economy is modelled as a 12-node Directed Acyclic Graph (DAG) using NetworkX.
A shock value is assigned to the root "Global Event" node and propagates through
weighted edges via topological sort — producing an impact score for each downstream
node (sectors and social classes). Edge weights (0.5–0.95) represent
empirically-derived transmission coefficients.
...
• Two-layer architecture: expert knowledge base + weighted DAG propagation (NetworkX)<br>
• Keyword-weighted scoring engine for custom scenario analysis (50+ economic tokens)<br>
• Multi-variable sensitivity analysis with 5 adjustable economic parameters<br>
• Validated against 3 real-world events — 30 sector-event pairs using MAPE metric<br>
